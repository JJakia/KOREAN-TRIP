  ...
  wrap.innerHTML = places.map((p, i) => {
    ...
    <div class="place-card" style="animation-delay:${i*0.03}s">
      ...
      <button class="naver-btn"
        ...
        data-url="${p.naverUrl || ''}">
        <span>🗺</span>
        <span>Naver<br>Map</span>
      </button>
    </div>`;
  }).join('');

  // Direct app launch on click
  wrap.querySelectorAll('.naver-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      openNaver(btn.dataset.name, btn.dataset.lat, btn.dataset.lng, btn.dataset.url);
    });
  });
}
...
/* ── OPEN NAVER MAP (direct app launch) ── */
function openNaver(name, lat, lng, directUrl) {
  const encoded = encodeURIComponent(name);

  // If user has set a direct Naver URL, use it
  if (directUrl) {
    window.open(directUrl, '_blank');
    return;
  }

  // Try Naver Map app scheme first (opens app directly if installed)
  const appScheme = `nmap://search?query=${encoded}&appname=com.seoul.trip`;
  const webUrl = `https://map.naver.com/p/search/${encoded}`;

  // If app opens, window loses focus → cancel fallback
